...
namespace HotspotDiscovery
{
    ...
    {
        private const int weightThreshold = 600;
        ...
        public ClusterAggregator()
        {
            ...
            ReadObjects("/Users/fatihakdag/Desktop/Clique paper/R/dataset/s1.csv");
            ReadClusters("/Users/fatihakdag/Desktop/Clique paper/R/s1-test-jp/s1-kmeans-clusters.txt");
            Console.WriteLine("Cluster count: " + clusters.Count);
            ReadClusterRewards("/Users/fatihakdag/Desktop/Clique paper/R/s1-test-jp/s1-kmeans-cwidths.txt");
        ...
        void ReCluster()
        {
            ...
            //find centroids
            //List<Tuple<float, float>> centroids = new List<Tuple<float, float>>();
            List<Cluster> newClusters = new List<Cluster>();
            foreach (var cls in clusters) {
                if (cls.IsInSolution) {
                    var centerX = cls.Objects.Average(o => o.Attributes[0]);
                    var centerY = cls.Objects.Average(o => o.Attributes[1]);
                    var center = new Tuple<float, float>(centerX, centerY);
                    //centroids.Add(center);
                    cls.center = center;
                    Console.WriteLine("Center of cluster " + cls.ClusterIndex + " is " + centerX + "," + centerY);
                    newClusters.Add(cls);
                }
            }
            //reset clusters
            foreach (var cls in newClusters) {
                cls.Objects = new List<ClusterObject>();
            }
            foreach (ClusterObject o in objects) {
                var minDist = float.MaxValue;
                var myCls = newClusters.FirstOrDefault();
                foreach (var cls in newClusters) {
                    var dist = GetDist(o.Attributes[0], o.Attributes[1], cls.center.Item1, cls.center.Item2);
                    if (dist < minDist) {
                        myCls = cls;
                        minDist = dist;
                    }
            ...
            //set label to label of centroid resave.
            var sb = new StringBuilder();
            foreach (var cls in newClusters) {
                foreach (var obj in cls.Objects) {
                    sb.AppendLine($"{obj.Attributes[0]}, {obj.Attributes[1]}, {cls.ClusterIndex}");
                }
            }
            SaveToFile(sb.ToString(), "result2.csv");

            Console.WriteLine("\n\n-------R Output 2------\n");
            var rARgs2 = ("--vanilla '/Users/fatihakdag/Desktop/Clique paper/R/R_code/siluet.R' " + GetOutputFolderFullPath() + "/result2.csv");
            Console.WriteLine(rARgs2);
            var rOutput2 = RunRFile(rARgs2);
            Console.WriteLine(rOutput2);
        }
